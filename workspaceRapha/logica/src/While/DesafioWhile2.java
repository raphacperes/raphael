...
public class DesafioWhile2 {
	...
	public static void main(String[] args) {
		...
		int contador = 0;
				...
				do 
				{
					chute = Integer.parseInt(JOptionPane.showInputDialog("Chutaí qual valor ! "));
					contador++;
				...
				System.out.println("Até que enfim " + contador + " tentativas" );
}	
}
		...
		JOptionPane.showInputDialog("Voce errou ! Tente um número menor");
		
		do {
			
		} while (valor1 < tent1) ;
		
		JOptionPane.showInputDialog("Voce errou ! Tente um número maior");
		...
		JOptionPane.showInputDialog("Parabéns o número está correto");
...
/* 
		...
		while (nome.length() <=5 || nome.length() >= 20 || nome.contains(" ")==false) {
			nome = JOptionPane.showInputDialog("Inválido. Digite o nome e Sobrenome");
		...
		while (idade <= 18 || idade >= 80) {
			idade = Short.parseShort(JOptionPane.showInputDialog("Inválido. A idade deve estar entre 18 e 80"));
